...
public class Restaurant1MarketRole extends Agent implements Restaurant1Market {
	...
	public Restaurant1MarketRole(String name, int steakAmount, int fishAmount, int chickenAmount) {
		super();
	...
	public void msgFoodOrder(Restaurant1CookRole c, List<Restaurant1FoodOrder> order) {
		orders.add(new Order(c, order));
		stateChanged();
	}
	
	public void msgHereIsPayment(Restaurant1CashierRole c, double payment) {
		money += payment;
		stateChanged();
	}
	
	public void msgCannotPayBill(Restaurant1CashierRole c, double amount) { //Change for extra credit.
	...
	private void processOrder(Order o) {
		...
		synchronized(orders) {
			for(int i = 0; i < o.orders.size(); i++) {
				Restaurant1FoodOrder tempOrder = o.orders.get(i);
	...
	private void sendBill(Order o) {
		...
		synchronized(o.orders) {
			for(Restaurant1FoodOrder fo : o.orders) {
	...
	public void addCashier(Restaurant1CashierRole c) {
		this.cashier = c;
	}
	...
	private class Order {
		Restaurant1CookRole c;
		List<Restaurant1FoodOrder> orders;
		orderState s;
		
		Order(Restaurant1CookRole c, List<Restaurant1FoodOrder> orders) {
			this.c = c;
			this.orders = orders;
